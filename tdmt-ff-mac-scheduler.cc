...
int TdMtFfMacScheduler::getQueueSize(uint16_t rnti)
{
	std::map<LteFlowId_t, FfMacSchedSapProvider::SchedDlRlcBufferReqParameters>::iterator it;
	int queueSize=0;
	for (it = m_rlcBufferReq.begin(); it != m_rlcBufferReq.end(); it++)
	{
		if ((*it).first.m_rnti == rnti)
			{queueSize=(*it).second.m_rlcTransmissionQueueSize;break;}
	}
	return queueSize;
}
...
void TdMtFfMacScheduler::DoSchedDlTriggerReq(
		...
		if (wbCqi != 0)
		{
			if (LcActivePerFlow(*it) > 0)
			{
				...
				for (uint8_t k = 0; k < nLayer; k++)
				{
					...
					transportBlockSize=m_amc->GetTbSizeFromMcs(mcs, rbgSize);
					achievableRate += transportBlockSize/ 8 / 0.001;	//This rate is expressed in bytes/second, because transportBlockSize is in bit
	...
	while (itMap != allocationMap.end()) {
		...
		//std::cout<<"b) "<<newEl.m_rnti<<std::endl;
...
double TdMtFfMacScheduler::EstimateDlSinr(uint16_t rnti, uint16_t rb)
{
	std::map<uint16_t, std::vector<double> >::iterator itCqi = m_ueCqi.find(rnti);
	if (itCqi == m_ueCqi.end())
	{
	...
	} else
	{
		...
		double sinrSum = 0;
		int sinrNum = 0;
		for (uint32_t i = 0; i < m_cschedCellConfig.m_dlBandwidth; i++)
		{
			double sinr = (*itCqi).second.at(i);
			if (sinr != NO_SINR) {
				sinrSum += sinr;
...
double TdMtFfMacScheduler::getAlphaFromRNTI(int RNTI)
{
		//1) See if the RNTI can be directly matched into the corresponding IMSI
		std::map<int,int>::iterator it;
		it=RNTI_IMSI_Map.find(RNTI);
		bool RNTI_found=!(it == RNTI_IMSI_Map.end());
		
		if (RNTI_found)
		{
			//It means I already know who this guy is, so I'm able to reply with the corresponding alpha!
			int IMSI=it->second;
			return alphas[IMSI-1];
		}
		
		//If I'm here, it means I havent been able to match the RNTI with the corresponding IMSI,
		//I should parse the map again hoping the information is now available!
		RNTI_IMSI_Map.clear();
