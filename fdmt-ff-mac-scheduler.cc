...
FdMtFfMacScheduler::DoSchedDlTriggerReq (const struct FfMacSchedSapProvider::SchedDlTriggerReqParameters& params)
{
	...
	for (int i = 0; i < rbgNum; i++)
	{
		...
		if (rbgMap.at (i) == false)
		{
			...
			double rcqiMax = 0.0;
			int userNumber=1;
			for (it = m_flowStatsDl.begin (); it != m_flowStatsDl.end (); it++, userNumber++)
			{
...
double FdMtFfMacScheduler::getAlphaFromRNTI(int RNTI)
{
...
int FdMtFfMacScheduler::getIMSI(int RNTI)
{
	std::map<int,int>::iterator it;
